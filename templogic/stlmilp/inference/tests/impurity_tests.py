...
class TestSTL(unittest.TestCase):
    ...
    def test_ext_inf_gain(self):
        ...
        npt.assert_almost_equal(
            impurity.ext_inf_gain(
                [1, 4, 0, 4.5], impurity.DEArgs(primitive_good, models, rho, traces, maxt)),
            -0.6869615765973234)

        npt.assert_almost_equal(
            impurity.ext_inf_gain(
                [1, 4, 0, 4.5], impurity.DEArgs(primitive_bad, models, rho, traces, maxt)),
            0.0)
    ...
    def test_opt_inf_gain(self):
        ...
        opts = [impurity.optimize_impurity(traces, primitive.copy(), rho) for
                primitive in prims]
